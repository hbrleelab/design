...
- **저장소 공개 여부** — 로고와 템플릿만 있으므로 공개해도 무방합니다.
  ...
  일일이 추가해야 합니다. **공개 권장.**
- **ACS 로고** — `assets/cm-logo-onnavy.png` 는 Chemistry of Materials 로고를
  브랜드 색으로 재색상한 것입니다. 저장소를 공개하기 전에 ACS 측에 사용
  가능 여부를 확인하시기 바랍니다.
- **연락처** — 현재 `hbrlee.unist@gmail.com` (임시). 확정되면 전 파일 일괄 교체가
  필요합니다.
